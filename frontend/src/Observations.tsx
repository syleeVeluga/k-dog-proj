...
export const analysisNames: Record<string, string> = {
  queued: '대기', running: '관찰 중', retry_wait: '재시도 대기', observed: '관찰 완료',
...
type ObservationRun = { run_id: string; input_revision: number; status: string; is_current: boolean;
  ...
  steps: { stage: string; attempt: number; status: string; retry_at: string | null; usage: Record<string, string | number | boolean> }[] };
...
export function Observations({ item, writable }: { item: Case; writable: boolean }) {
  ...
  return <section className="panel" aria-label="영상 관찰">
    <div className="section-title"><h2>영상·음성 관찰</h2><span className="tag">{current ? analysisNames[current.status] : '실행 전'}</span></div>
    <p className="fine">{data?.message ?? '관찰 상태 확인 중…'} 화면을 닫아도 백그라운드 처리는 계속됩니다. 항목 평가·점수 계산은 다음 단계입니다.</p>
    ...
    {writable && <fieldset disabled={busy}><div className="toolbar">
      <button className="primary" disabled={!item.consent?.video_analysis || !item.consent?.external_ai}
        onClick={() => void act(base, { expected_revision: item.input_revision, reuse_run_id: reuse && current ? current.run_id : null })}>관찰 시작</button>
      {current && <label className="check"><input type="checkbox" checked={reuse} disabled={active || !current.steps.some(s => s.stage === 'observe' && s.status === 'succeeded')}
        onChange={e => setReuse(e.target.checked)} />선택 실행의 성공 관찰 재사용</label>}
      {active && <button onClick={() => void act(`${base}/${current.run_id}/cancel`)}>관찰 중지</button>}
    ...
    {current && <>
      ...
      {current.steps.map((s, i) => <p className="fine" key={i}>{s.stage === 'prepare' ? '미디어 검사' : s.stage === 'integrate' ? '근거 통합' : '카메라 관찰'} · 시도 {s.attempt} · {({ succeeded: '완료', running: '처리 중', failed: '실패', retry_wait: '재시도 대기', abandoned: '중단' } as Record<string, string>)[s.status] ?? s.status}
