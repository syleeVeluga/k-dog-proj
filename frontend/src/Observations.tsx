...
import { VideoAssessments, type VideoAssessment, type Ledger } from './VideoAssessments';
...
type ObservationRun = ScoreData & { run_id: string; session_id: string; created_at: string; input_revision: number; status: string; is_current: boolean;
  evaluation_mode: string; video_assessments: VideoAssessment[]; ledgers: Ledger[];
...
const branchLabels: Record<string, string> = { dog: '반려견', owner: '보호자' };

// Per-video steps carry "{video_id}" or "{video_id}/{branch}", so name the file and branch.
function stepTarget(stage: string, key: string, item: Case) {
  if (!['ledger', 'observe', 'review_video'].includes(stage)) return '';
  const [videoId, branch] = key.split('/');
  const name = item.manifest.sessions.flatMap(s => s.videos).find(v => v.video_id === videoId)?.original_name ?? videoId;
...
export function Observations({ item, writable, view }: { item: Case; writable: boolean; view: 'analysis' | 'report' }) {
  ...
  return <section className={`panel${view === 'report' ? ' report-panel' : ''}`} id="analysis" aria-label={view === 'report' ? '보고서 결과' : '영상 관찰'}>
    ...
    {current && <>
      ...
      <details><summary>미디어 검사·단계별 처리 이력</summary>{current.media.map(m => <p className="fine" key={m.video_id}>{item.manifest.sessions.flatMap(s => s.videos).find(v => v.video_id === m.video_id)?.original_name} · {m.duration_sec.toFixed(1)}초 · {m.width}×{m.height} · {m.codec} · {m.audio_status === 'present' ? '오디오 트랙 있음' : '오디오 없음'}</p>)}
      {Object.entries(current.media_errors).map(([id, message]) => <p className="error" key={id}>{message}</p>)}
      {current.steps.map((s, i) => <p className="fine" key={i}>{s.stage === 'prepare' ? '미디어 검사' : s.stage === 'integrate' ? '항목별 근거 정리' : s.stage === 'survey' ? '설문 계산' : s.stage === 'report' ? '리포트 설명' : s.stage === 'ledger' ? '사건 원장' : s.stage === 'review_video' ? '불일치 항목 영상 재검토' : s.stage === 'evaluate' ? `${s.branch_key === 'dog' ? '반려견' : '보호자'} ${current.evaluation_mode === 'per_video' ? '결과 병합' : '평가'}` : current.evaluation_mode === 'per_video' ? '영상별 직접 평가' : '카메라 관찰'}{stepTarget(s.stage, s.branch_key, item)} · 시도 {s.attempt} · {({ succeeded: '완료', running: '처리 중', failed: '실패', retry_wait: '재시도 대기', abandoned: '중단' } as Record<string, string>)[s.status] ?? s.status}
