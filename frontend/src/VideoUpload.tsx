...
type Upload = { file: File; state: '대기' | '등록 중' | '저장됨' | '실패'; error: string };
export function VideoUpload({ item, session, refresh }: { item: Case; session: Session; refresh: (message?: string) => Promise<void> }) {
  ...
  async function upload() {
    ...
    try {
      ...
      for (const entry of pending) {
        ...
        try {
          const query = new URLSearchParams({ session_id: session.session_id, filename: entry.file.name, expected_revision: String(current.input_revision) });
  ...
  return <form onSubmit={e => { e.preventDefault(); void upload(); }}><fieldset disabled={busy}>
    <div className="toolbar"><label>영상 파일<input type="file" accept=".mp4,.mov,.m4v,.avi,.mkv,.webm" multiple onChange={e => {
      const chosen = Array.from(e.target.files ?? []).map(file => ({ file, state: '대기' as const, error: '' }));
    ...
    <p className="fine">{item.participant_id} · {item.dog_name} · {item.manifest.sessions.findIndex(s => s.session_id === session.session_id) + 1}차 촬영. 한 쌍의 영상 파일을 여러 개 등록할 수 있습니다. 실패 이후 파일은 대기하며 저장된 파일은 다시 보내지 않습니다.</p>
