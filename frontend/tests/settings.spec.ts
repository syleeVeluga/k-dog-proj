...
test('M5: developer draft, isolated schema trial, apply, restore and key secrecy at 360px', async ({ page }, testInfo) => {
  ...
  await expect(panel.getByText('초안을 저장했습니다. 운영 버전은 유지됩니다.')).toBeVisible();
  ...
  await expect(panel.getByText('선택 버전을 운영 적용·복원했습니다. 기존 분석은 고정 설정을 유지합니다.')).toBeVisible();
  ...
  await expect(panel.getByText('키를 보호하여 저장했습니다.')).toBeVisible();
  ...
  await expect(panel.getByText('키를 폐기했습니다.')).toBeVisible();
