...
M4까지 인증·입력·영상 검사·Gemini 관찰·반려견/보호자 병렬 평가·점수 집계·근거 조회·리포트 설명·사람 수정·대표 프레임·PDF/XLSX/CSV 내보내기를 실행할 수 있다. React 빌드를 FastAPI에서 제공하며 SQLite와 별도 worker를 사용한다. 실제 공급자 호출·관찰/평가/설명 품질은 샘플 제공 후 검증할 예정이다. Python 3.14, Node.js 24, `uv`와 npm, FFmpeg/ffprobe를 사용한다. 명령은 저장소 루트에서 시작한다.
...
M4는 **리포트 설명 공급자**를 별도로 선택한다. 같은 worker가 설명을 생성하고, 상세에서 점수/설명 수정과 원본 대표 프레임 선택을 제공한다. 목록의 **전체 내보내기**와 상세의 **개별 파일 내보내기**는 결과 제공 동의를 검사하고 당시 revision을 고정한다. PDF/Excel/CSV 출력은 AI를 재호출하지 않는다. [M4 실행·검증 기록](K-DOG_M4_구현기록_v1.0_20260906.md)에 권한·실패 재시도·미정 그래프·환경 변수·시각 검증 방법을 기록했다.
