...
2026-09-07부터 신규 분석은 **영상별 직접 평가 → 항목별 결과 병합**이 기본이다. 개발자 설정의 동일 이름 단계에서 Gemini 모델·프롬프트를 편집하며 `신규 분석 방식`에서 기존 경로와 비교 실행할 수 있다. 과거 run의 설정·산출물은 유지한다. 호출 수·재검토·영상별 결과 화면·검증 및 한계는 [영상별 평가 구현 기록](K-DOG_영상별평가_구현검증_v1.0_20260907.md)을 따른다. 아래 M2/M3 설명은 기존 방식의 이력이다.
