...
| 파일 전달 | 기존 메일·메신저에서 수동 전송, 앱은 파일 생성과 전달 이력 기록 | 설계 기본값. 자동 발송 연동은 범위 밖 |
...
| 역할 | 허용 기능 |
|---|---|
| 운영자 | 모든 참가자·예약·설문·영상 조회, 업로드·ID 수정, 분석·재시도, 결과 확인·수정, 개별·전체 PDF/Excel/CSV 내보내기, 수동 전달 이력 |
...
- 운영자가 이메일·메신저에서 전달할 파일 준비와 전달 이력 기록.
- PC 실행 우선, 클라우드 배포 가능한 공통 구조, 비동기 작업·복구, 비용 기록, 백업·복원, 샘플 시연.

### 5.2 후속 선택 범위

Word 출력, 이메일·메신저 자동 발송 연동, 복수 행사 운영 고도화, GPT·Claude 이중 평가, 원본 수작업 Excel과 완전히 같은 모양의 역출력.
...
8. **전달:** 운영자가 기존 이메일·메신저에서 참가자별 파일을 전달한다. 교수·운영자는 모든 참가자 결과 파일을 받을 수 있다. 앱에는 전달 여부·방법·일자를 수동 기록할 수 있다.
...
분석 상태는 **준비 필요 → 대기 → 분석 중 → 분석 완료**로 표시한다. 실패·일시 중지·취소·오래된 결과는 별도 상태로 관리한다. 검토 여부·내보내기 여부·전달 여부는 분석 상태와 분리하며 승인·반려 상태를 만들지 않는다.
...
- 연결 오류·동의 철회 시 추가 내보내기를 중지하고 관련 전달 이력을 표시한다. 이미 이메일·메신저로 보낸 파일은 앱에서 회수할 수 없으므로 운영자가 정정 파일을 보내거나 삭제를 요청한다.
- 전달 상태는 미전달·전달 기록됨·정정 필요로 관리한다. 수동 전달 기록을 실제 배달·읽음 확인으로 표시하지 않는다.
...
| 화면 | 주요 내용 | 주 동작 |
|---|---|---|
| 참가자 목록 | ID·이름·예약·설문·영상·분석·전달 상태, 검색·일자 필터 | 참가자 열기 |
...
| 결과 목록 | 전체 결과 요약, 선택 항목, PDF/Excel/CSV, 파일 버전·전달 여부 | 내보내기 |
...
모든 참가자의 결과를 화면과 파일에서 확인할 수 있다. 전체 요약에는 참가자 ID·이름·세션·처리 상태·4영역 비교·관찰 충족도·검토 여부·전달 상태를 넣고, 상세에는 55개 항목, 설문 원응답·환산값, 근거 시간·카메라, 수정 전후와 버전을 제공한다.
...
| 모든 참가자 | 참가자별 PDF ZIP, 전체 합본 PDF도 선택 가능 | `전체요약`, `영역비교`, `행동55항목`, `설문30문항`, `관찰근거`, `검토·전달이력`, `규칙안내` 시트로 통합 |
...
UTF-8 BOM, 일관된 열 이름, 공란 결측, 시간대가 있는 날짜를 사용하고 자유 텍스트의 수식 실행을 방지한다. 내보내기마다 대상·작성자·결과 버전·파일 목록을 남긴다. 운영자는 실제 전달 후 이메일/메신저 등 방법·전달일·전달 대상 참가자를 기록할 수 있다. 자동 발송·배달 확인·메시지 읽음 상태는 MVP에 포함하지 않는다.
...
교수는 시스템에서 즉시 조회하거나 운영자가 전달한 전체 PDF/Excel을 검토할 수 있다. 앱은 파일 생성과 전달 이력까지 관리하며 실제 이메일·메신저 발송은 운영자가 수행한다.
...
| Participant | 행사·참가자 ID, 보호자·반려견 정보, 접근 권한, 동의 기록 |
...
| ExportBatch / DeliveryRecord | 대상 참가자·파일·버전·내보낸 계정, 수동 전달 방법·일자·정정 상태 |
...
- 리포트 미리보기·대표 이미지 선택, 개별·전체 PDF/Excel/CSV 내보내기, 수동 전달 이력 기록.
...
- 영상·음성 분석, 외부 AI 전송, 결과 제공에 대한 참가자 동의 상태·문구 버전·시각을 기록한다.
- 동의 상태 미확인은 외부 API 전송 전에 표시하고 확인 대상으로 둔다. 연구 활용 등 추가 목적은 기본 평가 제공 동의와 구분한다.
...
- 삭제 요청 시 신규 분석·내보내기를 중지하고 원본·파생 파일·DB·원격 임시 파일을 추적해 삭제한다. 백업은 정해진 순환 기간에 제거하며 복원 시 삭제 목록을 재적용한다. 이미 외부로 전달한 파일은 직접 회수할 수 없으므로 전달 이력에 따라 운영자가 수신자에게 별도 조치한다.
...
| AT-16 | 자료 삭제와 백업 복원 | 추가 내보내기 중지, 삭제 대상 재노출 없음, 이미 전달한 파일의 후속 조치 구분 |
...
| AT-22 | 전달한 리포트 정정·재발행 | 새 파일 버전, 정정 필요 상태, 앱이 외부 파일 회수를 완료했다고 표시하지 않음 |
...
파일럿은 제안상 10~20쌍의 동의된 영상으로 시작한다. 가림·무음·잡음·짧은 사건·다중 카메라·다양한 체형과 털색을 포함하고, 기준 사례 중 일부는 복수 검토자가 독립 채점해 사람 사이의 차이도 기록한다. 프롬프트 조정에 사용한 사례와 최종 평가 사례는 구분한다.
...
- 개별·전체 PDF/Excel, CSV, 이메일·메신저 수동 전달, 리포트 6개 구성은 유지한다.
...
| 운영 | 초기 로컬/클라우드 선택, 계정 수, 보관·동의, 행사일·개발 일정 | 로컬 우선 제안·클라우드 대안 유지, 현재 참가자 외부 조회 없음 |
