...
5. **전처리** — 확정한 구간으로 기준 영상을 잘라 클립을 만든다. 「전처리」 메뉴에서 기준 파일·회차·규칙을 확인하고 「이 촬영 전처리 시작」을 누른다. 운영자·관리자가 실행하고 교수/검토자는 조회만 한다. 런처의 API·worker가 실행 중이어도 전처리 전용 잠금으로 실행하며, 다른 전처리 및 정리·복원과는 동시에 실행하지 않는다. 현재 규칙 preprocess-v1은 입장·혼자·낯선·재회의 **구간 시작 기준 처음 최대 5초**를 8 fps, 나머지를 2 fps로 처리하며 오디오는 유지한다. 자극 순간 전후 창 배치는 고객 확인 대기이며 현재 방식으로 확정된 것이 아니다.
