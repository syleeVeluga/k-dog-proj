...
2026-09-07: **영상별 직접 평가·항목별 병합**. 신규 실행은 개별 영상마다 전체 55항목과 원문 선택지를 Gemini에 제공하고, 서버가 항목별 결과를 병합한다. 불일치 항목만 한 차례 재검토하며 미해결 충돌은 보류한다. 영상별 결과·관찰 범위·측정·근거 재생을 추가했다. 기존 실행은 보존한다. [변경 계획](K-DOG_영상별평가_변경계획_v1.0_20260907.md)과 [구현·검증 기록](K-DOG_영상별평가_구현검증_v1.0_20260907.md)이 신규 파이프라인 기준이다.

2026-09-06: **전달 기록 제거**. 보고서는 파일 내보내기까지만 담당한다. 이메일·메신저 채널 선택과 수동 전달 이력·정정 필요 표시를 제거했다. Python 127개·브라우저 16개·production build를 통과했다. 범위·검증·한계는 [전달 기록 제거 기록](K-DOG_전달기록제거_v1.0_20260906.md)을 따르며, 아래 기록의 전달 이력 관련 내용은 이 변경으로 대체된다.

2026-09-06: **운영자·교수 UI/UX 개선**. 목록 갱신·업무 필터, 화면 내 근거 재생, 부분 채점 안내, 편집 초안 보존, 파일별 업로드 복구, 선택 내보내기 미리보기, 문단별 근거·전후 비교, 원본 설문 열 연결을 추가했다. 구현 범위·cold review·검증·한계는 [UI/UX 개선·검증 기록](K-DOG_UIUX_개선검증_v1.0_20260906.md)을 따른다.
