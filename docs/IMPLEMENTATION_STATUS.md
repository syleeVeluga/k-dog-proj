...
아래 M0 기록은 이전 단계에서 완료한 계약·원본 이관 이력이다. M1 저장/권한의 반영 범위와 남은 F-01–F-04 검증은 위 M1 기록을 우선 참조한다.
...
M2: FFmpeg 미디어 검사·Gemini 관찰 어댑터·worker와 관찰/근거 저장. M1 `intake-1.0`을 검사한 뒤 RunInput으로 고정한다. F-01 점유 토큰 조건부 산출물 채택, F-02 각 외부 호출 전 현재 동의 재확인, F-03 허용 재사용 manifest를 구현·검증한다. 실제 동의된 영상·공급자 키가 없으면 fixture 계약 시험과 실 API/품질 검증을 구분한다. 미정 계산 규칙은 계속 보류한다.
