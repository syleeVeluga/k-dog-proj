...
2026-09-07: **영상별 직접 평가·항목별 병합**. 신규 실행은 개별 영상마다 전체 55항목과 원문 선택지를 Gemini에 제공하고, 서버가 항목별 결과를 병합한다. 불일치 항목만 한 차례 재검토하며 미해결 충돌은 보류한다. 영상별 결과·관찰 범위·측정·근거 재생을 추가했다. 기존 실행은 보존한다. 변경 계획과 [구현·검증 기록](K-DOG_영상별평가_구현검증_v1.0_20260907.md)이 신규 파이프라인 기준이다.
