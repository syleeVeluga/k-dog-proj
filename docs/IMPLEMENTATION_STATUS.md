...
위 M1과 아래 M0 내용은 이전 단계에서 완료한 계약·원본 이관 이력이다. 현재 저장/권한·관찰·F-01–F-04 검증 범위는 맨 위 M2 기록을 우선 참조한다.
...
M3: 반려견·보호자 병렬 평가, Gemini/GPT/Claude 평가 어댑터, 확정 규칙의 프로그램 집계와 부분 결과 조회. M2의 허용 근거 목록과 버전·재사용 계약을 유지한다. 실제 동의된 영상·공급자 키가 제공되면 M2 실 API/근거 품질 검증을 별도로 수행한다. 미정 계산 규칙은 계속 보류한다.
