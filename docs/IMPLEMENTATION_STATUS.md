...
2026-09-06: **M0 저장소·계약 완료**. 시작 시 문서 4개와 저장소 지침뿐이었으며, 현재 원본 항목집·검증 타입·고정 사례·실행 환경을 구성했다. 이번 완료 범위는 타입·항목집 이관·미정 규칙·계약 검증까지다. UI, 인증, DB, worker 실행은 M1 이후에 구현한다.
...
- Python 도메인은 향후 FastAPI/worker가 공유한다. React와 저장 프로세스는 아직 없다. [개발 실행 안내](DEVELOPMENT.md)를 따른다.
...
M1: 인증·역할 분리, SQLite 핵심 5개 테이블, 참가자 목록/상세, 표준 설문 입력과 영상 파일 관리. 1명·2영상·설문을 등록한 뒤 앱을 재시작해 다시 조회하는 것을 완료 기준으로 한다. F-01–F-04의 DB/manifest 계약을 함께 반영하고 미정 계산 규칙은 계속 보류한다. 새 라이브러리는 설치·사용 전 최신 안정 버전과 호환성을 다시 인터넷에서 확인한다.
