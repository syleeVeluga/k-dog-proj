...
위 M1/M2와 아래 M0 내용은 이전 단계의 이력이다. 현재 평가·집계·재사용 범위는 맨 위 M3 기록, 미디어·관찰 범위는 M2 기록을 우선 참조한다.
...
M4: 리포트 설명·검토 수정·대표 이미지·PDF/Excel/CSV 개별·전체 내보내기와 export snapshot. 실제 동의된 영상·공급자 키가 제공되면 관찰/평가 실 API·근거/선택 품질 검증을 별도로 수행한다. 미정 계산 규칙은 계속 보류한다.
