...
4. 03 `여러쌍비교!D6:Z6` 수식의 수치 기대값과 R4~R6 채택 해석의 상태를 구분한 골든 테스트 (05 예비촬영 6쌍 값)
...
| R5 | 걷기 시행 유효성 = 3이면 동조율 「무효」(국면 수가 있으면 `Z6`과 동일, 빈칸이면 무효 우선 해석으로 다름) **그리고** 걷기 항목(BS-08·DOG-19·20·21) 을 SYN 집계에서 제외 | 03 3_보호자행동 E13 라벨 「이 시행의 걷기 항목과 동조율 무효」. 엑셀 `P6·Q6`은 제외하지 않음 | **라벨과 수식이 다름** — 라벨을 따르고 확인 |
| R6 | 무시 지시 준수 = 3이면 무시 항목(DOG-17·18)을 ATT 집계·애착 유형·적응 지표에서 제외 | 03 3_보호자행동 E12 라벨. 엑셀 `V6·M6`은 검사하지 않음 | 위와 같음 |
...
R5는 05 `3_보호자행동!K13=3`인 두 번째 쌍에서 발동한다. 현재 동조율은 invalid, SYN 제외 수는 3이다. 같은 쌍의 `2_개행동!K25`는 빈칸이므로 03 `여러쌍비교!Z6` 구조에 대입하면 빈칸이다. 숫자가 모두 없음이어도 상태는 다르며, 현재는 무효를 우선하는 해석이다. SYN의 세 원점수도 빈칸이어서 제외 전후 평균 숫자만으로는 이 규칙을 검증할 수 없다. R6의 05 `3_보호자행동!J12:O12`는 모두 빈칸이므로 발동하지 않는다. 합성 시험은 국면 수 있음/없음과 시행 유효성 1/2/3/미판독을 별도 검증한다. R4는 `2_개행동!J20=0`, K20:O20은 빈칸이며 다른 유형 재료도 결측이어서 6쌍만으로 모든 경계를 판별할 수 없다.
...
`backend/tests/test_scoring_golden.py` (신설) — 05 `예비촬영_6쌍_채점_20260913.xlsx`의 J~O열(6쌍)을 읽어 `ScoreSheet`를 만든다(빈칸 → `unreadable`, 사유 「05 빈칸」). 수치 기대값은 03 `여러쌍비교!D6:Z6`을 손으로 계산한 상수이며, R4~R6의 문서·라벨 해석으로 달리 정한 상태를 별도로 기록한다. Excel 엔진 재계산 시험이 아니다.
...
`backend/tests/test_evaluation.py`: import 경로만 변경(legacy). 완료 기준: 골든 6쌍의 수치 및 별도 해석 상태 기대값 전부 일치, 신규 시험 통과, 기존 시험 통과.
...
- PR-4: 05 예비촬영 6쌍 골든 테스트가 03 수식에서 손계산한 수치와 R4~R6 채택 해석의 상태 기대값에 일치. 수식과 다른 상태는 고객 회신 대기다. **이것이 변경검토 6장 P0의 완료 기준이다.**
