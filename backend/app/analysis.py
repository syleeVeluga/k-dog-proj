...
def view_analysis(store, case_id):
    ...
    return AnalysisView(configured=configured(), message="관찰 설정 준비됨 · 실제 연결은 실행 시 확인합니다." if configured()
...
def control(store, case_id, run_id, actor, action):
    with store.connect(write=True) as db:
        ...
        if action == "retry":
            ...
            steps = db.execute("SELECT branch_key,stage,MAX(attempt) AS attempts FROM steps WHERE run_id=? GROUP BY stage,branch_key",
                               (run_id,)).fetchall()
            if any(step["attempts"] >= 3 for step in steps):
                raise HTTPException(409, "실행 시도 한도에 도달했습니다. 입력·설정을 확인하고 새로 실행하세요.")
            repairs = {}
            for step in db.execute("SELECT branch_key,usage_json FROM steps WHERE run_id=? AND stage='observe'", (run_id,)):
                if json.loads(step["usage_json"]).get("code") == "observation_schema_invalid":
                    repairs[step["branch_key"]] = repairs.get(step["branch_key"], 0) + 1
            if any(count >= 2 for count in repairs.values()):
                raise HTTPException(409, "관찰 구조 수정 한도에 도달했습니다. 입력·설정을 확인하세요.")
            state = "queued"
        else:
